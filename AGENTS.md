...
> **YOU MUST COMPLETE THIS CHECKLIST BEFORE WRITING A SINGLE LINE OF CODE.**
> **NO EXCEPTIONS. NO SHORTCUTS. NO SKIPPING.**

---

### ❌ DO NOT GENERATE ANY CODE UNTIL YOU HAVE DONE THE FOLLOWING:

**Step 1 — Identify which docs apply to your task:**

| Task involves... | Read this file FIRST |
|---|---|
| Authentication, auth state, route protection, user data, Clerk | [`docs/auth.md`](docs/auth.md) |
| UI components, forms, buttons, inputs, styled elements, shadcn | [`docs/ui-components.md`](docs/ui-components.md) |

**Step 2 — Open and READ the entire relevant file(s) using your file-reading tools.**

**Step 3 — Only after reading, proceed with code generation that follows those exact patterns.**

---

### Why This Is Non-Negotiable

Every relevant `/docs` file contains project-specific patterns, conventions, and security requirements that **cannot be inferred from the codebase alone**. Skipping this step **will** cause:

- **Security vulnerabilities** from incorrect auth/authorization implementations
- **Broken UI** from components that don't follow the project's design system
- **Wasted effort** rewriting code that doesn't match established patterns
- **Inconsistency** that degrades the codebase over time

> **If you are an AI agent reading this:** treat the `/docs` files as authoritative specifications. Reading them is not optional prep work — it is the first required step of every task. Use your file-reading tool on each relevant file before outputting any code.

---

### Required Documentation by Topic

- **🔐 [Authentication Guidelines](docs/auth.md)** — **READ THIS COMPLETELY** before implementing any authentication, route protection, user data access, Clerk hooks, middleware, or auth-related features
- **🎨 [UI Components Guidelines](docs/ui-components.md)** — **READ THIS COMPLETELY** before creating or installing any UI components, forms, buttons, inputs, or styled elements

**Mandatory Workflow — follow in order, every time:**

1. Identify what you need to implement
2. Determine which `/docs` files apply (see table above)
3. **Read each relevant file in full using your file-reading tool**
4. Confirm you understand the patterns before writing any code
5. Generate code that strictly follows those patterns
